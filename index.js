...
async function flashFirmware(options) {
  ...
  console.log(`正在烧录固件: ${resolvedPath}`);
  console.log(`目标芯片: ${target}, 接口: ${iface}, 传输: ${transport}`);
  ...
  if (success) {
    console.log("烧录成功！");
    if (verify && output.includes("Verified OK")) {
      console.log("校验通过。");
    }
  } else {
    console.error("烧录失败！");
...
async function detectAllDevices() {
  ...
  if (allDevices.length === 0) {
    ...
    console.log(`共检测到 ${allDevices.length} 个调试器设备：\n`);
